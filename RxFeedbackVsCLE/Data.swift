...
func loadSomeData(page: Int, limit: Int) -> Observable<[String]> {
  Observable.just(())
    .flatMap { _ -> Observable<[String]> in
      return .just(text(page: page, count: limit))
    }
    .delay(.microseconds(300), scheduler: ConcurrentDispatchQueueScheduler(qos: .background))
}

private func text(page: Int, count: Int) -> [String] {
  Array(1...count)
    .map { "\((page - 1) * count + $0) - \(randomWords())"}
}

private func randomWords() -> String {
  Array(0...Int.random(in: 3...10))
    .map { _ in randomWord(wordLength: Int.random(in: 4...10)) }
    .joined(separator: " ")
}
